...
Before every hypothesis, we should check if investigated data in hypothesis are normally distributed or not. This is crutial information before choosing right classification model for our hypothesis.
...
```{r}
ad.test(class_data$danceability)$p.value
```
...
```{r}
hist(class_data$danceability, col = "green")
```
...
```{r}
model_glm <- glm(mode ~ valence, data = train_data, family = binomial)
plot(model_glm, which = 1)
summary(model_glm)
```
TODO opisat summary a plot

Plot: A Pearson residual close to zero indicates that the observed response is close to the predicted probability, suggesting a good fit between the model and the data.
Positive Pearson residuals indicate that the observed response is higher than predicted, while negative Pearson residuals indicate that the observed response is lower than predicted.

```{r}
coef(model_glm)
```
**Negative coefficient** suggest, that lower danceability are associated with higher likelihood of being *major mode*.
So if coefficient for *danceability* is -1.172, it implies that for every unit of *mode* it decreases in *danceability*.

We can see this decrease in also in this plot.
```{r}
plot(class_data$danceability, class_data$mode, xlab = "Danceability", ylab = "Mode", pch = 19, col = ifelse(class_data$mode == 1, "green", "red"))
...
Now we have model, so we can predict how well trained is model with test data.
But before that, we must choose cut-off (TODO co je cut-off??). We can get optimal cut-off with help of **ROC curves**.
```{r}
...
Now we can get prediction classes with optimal cut-off and draw ROC curve to see TODO co??
```{r}
...
P <- TP + FN # all positives
N <- FP + TN # all negatives
...
A higher precision indicates fewer false positives, meaning fewer instances are incorrectly predicted as *major mode*.
```{r}
precision <- TP / (FP + TP)
print(paste("Precision: ", precision))
```
**Recall** measures the ability of the model to capture all actual positives.
A higher recall indicates that the model captures a higher proportion of actual *major mode* instances.
```{r}
recall <- TP / P
